...
import { fetchHoldToken, bookSeats, fetchStadiumBookings } from "../services/SeatsService";
...
const SeatsChart = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const bookings = await fetchStadiumBookings(SEATS_CONFIG.Secretworkspacekey, SEATS_CONFIG.eventkey);
        console.log("Fetched Bookings:", bookings); // Debugging log
        setStadiumBookings(bookings);
      } catch (error) {
  ...
  return (
    <div className="seats-chart-container">
      ...
      <div className="stadium-booking-box">
        <h3>Booking Details</h3>
        <div className="booking-list">
          {stadiumBookings.length > 0 ? (
            stadiumBookings.map((seat) => (
              <div key={seat.objectId} className="booking-item">
                {seat.label} - {seat.categoryLabel}
              
                {seat.availabilityReason}
                <h5>categoryKey</h5>
                {seat.categoryKey}
